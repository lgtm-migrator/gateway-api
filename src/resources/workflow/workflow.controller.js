...
module.exports = {
	...
	updateWorkflow: async (req, res) => {
		try {
			// 1. Look at the payload for the publisher passed
			let workflow = new WorkflowModel({ test: 'lol' });

			workflow.save();

			// 2. Check if the user is a manager of that team

			// 3. Create new workflow model

			// 4. Return workflow payload
			...
			return res.status(500).json({
				success: false,
				message: 'An error occurred updating the workflow',
			});
		}
	},
