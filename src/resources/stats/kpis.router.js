...
router.get('', async (req, res) => {
	...
	switch (req.query.kpi) {
		...
		case 'topdatasets':
				...
				.then(async data => {
					for (let datasetIdObject in data) {
						if (data[datasetIdObject].datasetIds && data[datasetIdObject].datasetIds.length > 0) {
								...
								if (!hdrDatasetIds.includes(data[datasetIdObject].datasetIds[datasetId])) {
									...
									if (result.length > 0) {
										if (DarInfoMap.has(data[datasetIdObject].datasetIds[datasetId])) {
											...
											count.requests++;
											DarInfoMap.set(data[datasetIdObject].datasetIds[datasetId], {
												requests: count.requests,
												name: result[0].name,
												publisher: result[0].datasetfields.publisher,
											});
										} else {
											DarInfoMap.set(data[datasetIdObject].datasetIds[datasetId], {
												requests: 1,
							...
							data[datasetIdObject].dataSetId &&
							data[datasetIdObject].dataSetId.length > 0 &&
							!hdrDatasetIds.includes(data[datasetIdObject].dataSetId)
						) {
							...
							if (result.length > 0) {
								if (DarInfoMap.has(data[datasetIdObject].dataSetId)) {
									...
									count.requests++;
									DarInfoMap.set(data[datasetIdObject].dataSetId, {
										requests: count.requests,
										name: result[0].name,
										publisher: result[0].datasetfields.publisher,
									});
			...
			let sortedResults = Array.from(DarInfoMap).sort((a, b) => {
