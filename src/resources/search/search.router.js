...
router.get('/', async (req, res) => {
	...
	const summaryCounts = await Promise.all([
		...
		getObjectCount('cohort', searchAll, getObjectFilters(searchQuery, req, 'cohort')),
	]);
	...
	const summary = {
		...
		cohortCount: summaryCounts[7][0] !== undefined ? summaryCounts[7][0].count : 0,
	};
	...
	recordSearchData.returned.cohort = summaryCounts[7][0] !== undefined ? summaryCounts[7][0].count : 0;
	...
	if (tab === '') {
		return res.json({
			...
			cohortResults: allResults[7].data,
			summary: summary,
