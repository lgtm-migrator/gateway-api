...
describe('Search API', () => {
	...
	['annual district death daily', 'cancer', 'data'].forEach(function (searchString) {
		test(`Search for string '${searchString}', first dataset result should contain name or description '${searchString}'`, async () => {
			...
			let expected = [expect.stringMatching(searchString.toLowerCase())];

			expect([name.toLowerCase(), description.toLowerCase(), keywords.toLowerCase()]).toEqual(expect.arrayContaining(expected));
		});
	});
