...
router.get('/datasets/search', async (req, res) => {
  ...
  if (req.query.search) {
    ...
    // axios.get(metadataCatalogue + '/api/catalogueItems/search?searchTerm=&domainTypes=DataModel')
    axios.get(metadataCatalogue + '/api/catalogueItems/search?searchTerm=' + searchString + '&domainTypes=DataModel')
...
// router.get('/datasets/search/:searchString', async (req, res) => {
//   var metadataCatalogue = process.env.metadataURL || 'https://metadata-catalogue.org/hdruk';

//      axios.get(metadataCatalogue + '/api/catalogueItems/search?searchTerm=' + req.params.searchString + '&domainTypes=DataModel')
//     .then(function (response) {
//       // handle success
//       return res.json({ 'success': true, 'data': response.data });
//     })
...
//       return res.json({ success: false, error: err.message + ' (raw message from metadata catalogue)' });
//     })
//     console.log('in search with param')
// });

// router.get('/dataset/search', async (req, res) => {
//   var metadataCatalogue = process.env.metadataURL || 'https://metadata-catalogue.org/hdruk';
//   var searchString = "";
...
router.get('/datasets/detail/:id', async (req, res) => {
  var metadataCatalogue = process.env.metadataURL || 'https://metadata-catalogue.org/hdruk';
...
router.get('/datasetfilters', async (req, res) => {
    var modelCatalogue = 
    //How do you add a new var into the .env file?
    // process.env.metadataURL || 
    'https://modelcatalogue.cs.ox.ac.uk/hdruk-preprod';
  
    axios.get(modelCatalogue + '/api/dataModels/profile/values/uk.ac.hdrukgateway/HdrUkProfilePluginService?filter=publisher&filter=license&filter=geographicCoverage&filter=ageBand&filter=physicalSampleAvailability&filter=keywords')
      .then(function (response) {
        // handle success
        return res.json({ 'success': true, 'data': response.data });
      })
...
// router.get('/datasetfilters', async (req, res) => {
router.get('/datasetfilters/:searchString', async (req, res) => {
  var modelCatalogue = 
  //How do you add a new var into the .env file?
  // process.env.metadataURL || 
  'https://modelcatalogue.cs.ox.ac.uk/hdruk-preprod';
  ...
  // axios.get(modelCatalogue + '/api/dataModels/profile/values/uk.ac.hdrukgateway/HdrUkProfilePluginService?filter=publisher&filter=license&filter=geographicCoverage&filter=ageBand&filter=physicalSampleAvailability&filter=keywords')
  axios.get(modelCatalogue + '/api/dataModels/profile/values/uk.ac.hdrukgateway/HdrUkProfilePluginService?filter=publisher&filter=license&filter=geographicCoverage&filter=ageBand&filter=physicalSampleAvailability&filter=keywords&search=' + req.params.searchString)
    .then(function (response) {
      // handle success
      return res.json({ 'success': true, 'data': response.data });
    })
    .catch(function (err) {
      // handle error
      return res.json({ success: false, error: err.message + ' (raw message from model catalogue)' });
    })
